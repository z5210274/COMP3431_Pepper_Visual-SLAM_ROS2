...
bool naoqi_bridge_msgs__msg__joint_angle_trajectory__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("naoqi_bridge_msgs.msg._joint_angle_trajectory.JointAngleTrajectory", full_classname_dest, 66) == 0);
  }
  naoqi_bridge_msgs__msg__JointAngleTrajectory * ros_message = _ros_message;
  {  // header
  ...
  {  // times
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      float * dest = ros_message->times.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
{
  /* NOTE(esteve): Call constructor of JointAngleTrajectory */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("naoqi_bridge_msgs.msg._joint_angle_trajectory");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "JointAngleTrajectory");
  ...
  {  // times
    ...
    if (ros_message->times.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      float * src = &(ros_message->times.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->times.size * sizeof(float));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
